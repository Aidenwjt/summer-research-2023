...
The applied sections of these notes will mainly follow Nochetto and Veeser's \textit{Primer of Adaptive Finite Element Methods} \cite{Nochetto2012} survey,
as well as Bartels'
...
\begin{definition}[Continuous embedding]
	...
	\textit{continuously embedded} in $Y$.
\end{definition}

\begin{definition}[Compact embedding]
	Let $X$ and $Y$ be as previously defined.
	We say that $X$ is \textit{compactly embedded} in $Y$, written
...
\begin{motivation}
	Some of the previous Sobolev embeddings can actually be improved to compact embeddings.
	...
	I will use Gilbarg and Trudinger's version here as Evans doesn't mention the second point.
\end{motivation}

\begin{theorem}[Rellich-Kondrachov compactness theorem]
	Let $U \subset \mathbb{R}^n$ be a bouned open set. The spaces $W_0^{k,p}(U)$ are compactly embedded in
	\begin{enumerate}[(i)]
		\item $L^q(U)$ for any $q < \frac{np}{n-p}$, if $p<n$, and
