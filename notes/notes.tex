...
\begin{definition}
	If $1 \leq p < n$, then the \textit{Sobolev conjugate} of $p$ is
	\begin{equation*}
		p^* = \frac{np}{n-p}.
	\end{equation*}
\end{definition}
...
\begin{definition}
	...
	The $i^{th}$ \textit{difference quotient} of size $h$ is
	\begin{equation*}
...
\section{Notation}
\begin{enumerate}[(i)]
	\item A \textit{multi-index} is a vector $\alpha = (\alpha_1, \dots, \alpha_n)$ where each component $\alpha_i \in \mathbb{N}_0$.
